...
export class ShellTool {
    ...
    async run(command: string): Promise<ExecutionResult<{ stdout: string; stderr: string }>> {
        ...
        return this.core.execute(async () => {
            // Added 30s timeout and 2MB buffer limit to prevent DoS via resource exhaustion
            const { stdout, stderr } = await execAsync(command, { timeout: 30000, maxBuffer: 1024 * 1024 * 2 });
